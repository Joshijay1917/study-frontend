  ...
  return (
    ...
    <div className="flex-1 relative">
      <div className="absolute inset-y-0 top-[12%] left-0 right-0 app-pages-theme rounded-tl-[80px] rounded-tr-[0px] shadow-lg overflow-auto">
        <div className='p-5'>
          <div className='text-3xl px-2 mt-5 flex items-center gap-3'>
          <FaHistory className='icon-theme mt-1'/>
          <h1 className='font-bold text-gray-800'>Latest Updates</h1>
          </div>
          {(!data?.data || data?.data.length === 0) && <p className='text-red-500 flex items-center gap-2 font-semibold text-nowrap justify-center py-10'><MdReportProblem className='text-yellow-600 text-2xl'/> No Updates are available!! <MdReportProblem className='text-yellow-600 text-2xl'/></p>}
          {data?.data && data.data.map(update => (
            <div key={update._id} onClick={()=>handleClick(update)} className='mt-5 cursor-pointer shadow-lg border border-gray-300 p-3 font-semibold relative pl-7 bg-[#2222] rounded-2xl'>
