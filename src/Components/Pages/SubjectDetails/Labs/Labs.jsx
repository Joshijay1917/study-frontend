...
import { useDeleteLabmanualMutation, useGetAllLabsQuery } from '../../../../Redux/Features/ApiSlice'
...
const Labs = ({ setlabform, setloading }) => {
  ...
  const [deleteLabmanual, deleteLabmanualOptions] = useDeleteLabmanualMutation()
    ...
    <div>
      ...
      <div className='mt-10'>
        ...
        {user.username === 'admin' && <div className='flex flex-col gap-2 items-center mt-10'>
          ...
          <button onClick={() => setlabform(true)} className='flex w-fit items-center justify-center bg-theme px-10 p-2 rounded-2xl text-white'><FaPlus /> Add</button>
        </div>}
      </div>
    </div>
  )
